...
if st.session_state.informe_actual:
    ...
    with st.sidebar.form("contract_form"):
        ...
        if submitted:
            ...
            # Preparamos el contexto para Jinja2
            # Preparamos el contexto para Jinja2
            context = {
                "nombre_afiliado": c_nombre,
                "rut_afiliado": c_rut,
                "direccion_afiliado": c_direccion if c_direccion else "________________",
                "comuna_afiliado": c_comuna if c_comuna else "________________",
                "ciudad_afiliado": c_ciudad if c_ciudad else "________________",
                "telefono_afiliado": c_telefono if c_telefono else "________________",
                "celular_afiliado": c_celular if c_celular else c_telefono,
                "correo_afiliado": c_email if c_email else "________________",
                "estado_civil_afiliado": c_estado_civil,
                "fecha_nacimiento_afiliado": c_fecha_nac,
                "oficio_afiliado": c_oficio,
                "afp_origen": c_afp,
                "sistema_salud": c_salud,
                "tipo_pension": c_tipo_pension,
                "fecha_actual": datetime.now().strftime("%d de %B de %Y")
            }
            
            # Datos específicos de Sobrevivencia
            if tipo_contrato_sel == "Sobrevivencia":
                 context["nombre_causante"] = c_causante_nombre
                 context["rut_causante"] = c_causante_rut
                 context["nombre_consultante"] = c_consultante_nombre
                 context["rut_consultante"] = c_consultante_rut
                 # Default placeholders since we don't extract these yet
                 context["profesion_consultante"] = "________________"
                 context["estado_civil_consultante"] = "________________"
                 context["fecha_nacimiento_consultante"] = "________________"

            # Recopilación de Beneficiarios (Lista de dicts para el loop de Jinja)
            beneficiaries_list = []
            
            # INTENTO 1: Extraer desde el Informe Generado (Prioridad)
            if tipo_contrato_sel == "Sobrevivencia" and st.session_state.get("informe_actual"):
                from contract_utils import extract_beneficiaries_from_report
                extracted_bens = extract_beneficiaries_from_report(st.session_state.informe_actual)
                if extracted_bens:
                    beneficiaries_list = extracted_bens
                    # Si faltan datos en el reporte (ej. parentesco), podríamos intentar completarlos con el form, pero es complejo mezclar.
                    # Asumimos que el reporte está completo si la tabla existe.
            
            # INTENTO 2: Si no se extrajo nada, usar los del Formulario/SessionState (Fallback)
            if tipo_contrato_sel == "Sobrevivencia" and not beneficiaries_list:
                # 1. Beneficiario Principal (del Formulario)
                if c_ben_nombre:
                    ben1 = {
                        "nombre": c_ben_nombre,
                        "rut": c_ben_rut,
                        "parentesco": c_ben_parentesco,
                    }
                    beneficiaries_list.append(ben1)
                
                # 2. Otros Beneficiarios (del Session State, desde el 2 en adelante)
                for i in range(2, 6): # Revisamos hasta 5
                    if f"Beneficiario {i} Nombre" in st.session_state.contract_data:
                        ben_next = {
                            "nombre": st.session_state.contract_data.get(f"Beneficiario {i} Nombre", ""),
                            "rut": st.session_state.contract_data.get(f"Beneficiario {i} RUT", ""),
                            "parentesco": st.session_state.contract_data.get(f"Beneficiario {i} Parentesco", ""),
                        }
                        if ben_next["nombre"]:
                            beneficiaries_list.append(ben_next)
            
            # Agregamos la lista al contexto (aunque contract_utils lo hace también, es bueno ser explícito)
            context["beneficiaries"] = beneficiaries_list

            template_file = get_contract_template_path(tipo_contrato_sel)
            
            with st.spinner("Generando contrato inteligente..."):
                try:
                    # Llamamos a la nueva función que usa DocxTpl
                    # Nota: Pasamos 'context' como segundo argumento. El tercero es opcional pero lo dejamos limpio.
                    docx_bytes = generate_contract_docx(template_file, context, beneficiaries_list)
                    ...
                    st.success("¡Contrato Inteligente Generado!")
                except Exception as e:
                    st.error(f"Error: {e}")
