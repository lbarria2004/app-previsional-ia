...
def _replace_text_in_paragraph(paragraph: Paragraph, replacements: Dict[str, str]) -> None:
    ...
    for key, value in replacements.items():
        if key in text:
            # We allow substituting with empty string, so we don't check 'if value'
...
def _fill_beneficiary_table(doc: Document, beneficiaries: list) -> None:
    """
    Fills the beneficiary 'table' (which is actually tab-separated text rows) 
    in the document with the provided data.
    """
    if not beneficiaries:
        return

    # Identify the "data rows" for beneficiaries.
    # The template apparently uses paragraphs with tabs, not a real Word Table.
    # We look for paragraphs containing the placeholder "{NOMBRE BENEFICIARIO}".
    
    beneficiary_paragraphs = []
    
    for p in doc.paragraphs:
        if "{NOMBRE BENEFICIARIO}" in p.text:
            beneficiary_paragraphs.append(p)
            
    if not beneficiary_paragraphs:
        logger.warning("No beneficiary placeholders ({NOMBRE BENEFICIARIO}) found in document paragraphs.")
        return

    # Iterate through the found paragraphs and fill or clear them
    for i, paragraph in enumerate(beneficiary_paragraphs):
        if i < len(beneficiaries):
            # We have data for this row
            ben_data = beneficiaries[i]
            _replace_text_in_paragraph(paragraph, ben_data)
        else:
            # No data for this row, clear the placeholders
            empty_data = {
                "{NOMBRE BENEFICIARIO}": "",
                "{RUT BENEFICIARIO}": "",
                ...
                # Also generic placeholders if they exist specifically in these rows
                "{F o M}": "", 
                "{SI o No}": ""
            }
