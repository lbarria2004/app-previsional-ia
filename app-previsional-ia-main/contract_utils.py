...
def fill_beneficiary_placeholders(doc: DocxTemplate, beneficiaries: List[Dict[str, str]]):
    ...
    for table in tables:
        for row in table.rows:
            # Check if this row is a "beneficiary row"
            # We check for the presence of the primary placeholder
            row_text = "".join(cell.text for cell in row.cells)
            
            if "{NOMBRE BENEFICIARIO}" in row_text:
                # Determine which beneficiary data to use
                if ben_idx < len(beneficiaries):
                    ben_data = beneficiaries[ben_idx]
                    ben_idx += 1
                else:
                    # No more beneficiaries, use empty strings to clear placeholders
                    ben_data = {k: "" for k in ["nombre", "rut", "fecha_nacimiento", "parentesco", "sexo", "invalidez"]}
                
                # Perform replacement in all cells of this row
                for cell in row.cells:
                    for paragraph in cell.paragraphs:
                        # Iterate runs to preserve formatting if possible, 
                        # but simple text replacement is safer for exact string matching across runs.
                        # Given the specific nature of these placeholders, they might be split across runs.
                        # For robustness, we will do a simple text replacement on the paragraph.text
                        # NOTE: This resets run formatting (bold/italic) for the whole paragraph.
                        # If preserving formatting is critical, we'd need complex run iteration.
                        # Assuming the placeholders are unique words, we try run-based first.
                        
                        modified = False
                        for run in paragraph.runs:
                            for ph, key in placeholders_map.items():
                                if ph in run.text:
                                    val = ben_data.get(key, "")
                                    # If val is None, use empty
                                    if val is None: val = ""
                                    run.text = run.text.replace(ph, str(val))
                                    modified = True
                        
                        # Fallback: if placeholders exist but weren't in single runs (split),
                        # we force replace on paragraph text (losing formatting).
